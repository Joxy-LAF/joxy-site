...
We composed a page with instructions about that here: <a class="alink" href="/p/joxy/wiki/Using%20Joxy/">[Using Joxy]</a>.</p>
...
The current version is 0.1.0. The first digit is a 0, because Joxy is not stable yet. We just increment the last digit for every new version and if there is a large bugfix release, we increment the middle digit. So the versions with a 0 in the end are meant to be more stable, but in this early development phase we still recommend using the latest version.</p>
<p><strong>When will the next version of Joxy be released?</strong><br />
The following Joxy version will probably be 0.1.1, containing new features.</p>
<p><strong>How can I help with improving Joxy?</strong><br />
The best way is probably by testing applications with Joxy, and reporting bugs.</p>
<p><strong>What systems are supported?</strong><br />
Firstly, only KDE systems on Linux are supported at the moment.<br />
We only test Joxy on two systems (Debian Testing and Ubuntu 12.04), but it should work with others. If you find problems please report them. Also, if you use another distribution and the installation/compilation instructions are incorrect for this distribution, please report that too, so we can change them.</p>
